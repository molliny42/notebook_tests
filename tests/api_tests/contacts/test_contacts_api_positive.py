...
def test_add_contact(contacts_api_client):
    ...
    with allure.step(f"Add a new contact: {contact_data}"):
        ...
        response = contacts_api_client.add_contact(contact_data.to_dict())  # Используем to_dict()

    ApiResponseHelper.check_response(response, 201, logger)  # Проверяем статус код
    response_data = response.json()
    ApiResponseHelper.check_user_data(contact_data, response_data, logger)  # Проверяем данные пользователя
...
def test_get_contacts(contacts_api_client):
    ...
    ApiResponseHelper.check_response(response, 200, logger)  # Проверяем статус код
    response_data = response.json()
...
def test_get_contact_by_id(contacts_api_client):
    ...
    with allure.step("Add a new contact for receiving"):
        ...
        add_response = contacts_api_client.add_contact(contact_data.to_dict())  # Используем to_dict()
    ...
    ApiResponseHelper.check_response(get_response, 200, logger)  # Проверяем статус код
...
def test_delete_contact(contacts_api_client):
    ...
    ApiResponseHelper.check_response(delete_response, 200, logger)  # Проверяем статус код
